<template>
  <transition name="fade" appear>
    <div
      v-if="modelValue"
      class="fixed z-20 inset-0 bg-gray-900 bg-opacity-50 transition-opacity backdrop-grayscale backdrop-filter"
      aria-hidden="true"
      @click.stop="onClose"
    ></div>
  </transition>
  <transition name="slide-open" appear>
    ...
    >
      <div
        class="flex h-full items-center justify-center pt-4 px-4 pb-20 text-center"
      >
        <div
          v-if="modelValue"
        ...
        <div
          class="z-50 inline-block align-bottom dark:bg-gray-700 bg-white dark:text-white text-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform p-1 min-w-[20rem] transition-all max-h-[85vh] relative flex flex-col"
        >
